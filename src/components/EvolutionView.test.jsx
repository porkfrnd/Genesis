...
describe('EvolutionView', () => {
  ...
  it('confirms a saved run in the archive panel', () => {
    const onSave = vi.fn(() => ({ name: 'Cold archive' }));
    render(<EvolutionView snapshot={{ seed: 42, generation: 2, history, stats: history[1] }} onSave={onSave} experiments={[]} onLoad={() => {}} onDelete={() => {}} />);
    fireEvent.change(screen.getByLabelText('Name this run'), { target: { value: 'Cold archive' } });
    fireEvent.click(screen.getByRole('button', { name: 'Save current state' }));
    expect(onSave).toHaveBeenCalledWith('Cold archive');
    expect(screen.getByRole('status')).toHaveTextContent('Saved Cold archive.');
  });
});
