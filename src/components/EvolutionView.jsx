...
export default function EvolutionView({ snapshot, onSave, experiments, onLoad, onDelete }) {
  ...
  const deltas = traits.map((key) => ({ key, delta: (current.averages?.[key] ?? 0) - (initial.averages?.[key] ?? 0), before: initial.averages?.[key] ?? 0, after: current.averages?.[key] ?? 0 }));
  ...
  const explanation = current.extinct ? `The population reached extinction at generation ${current.generation}.` : `${largestShift.key === 'coldTolerance' ? 'Cold tolerance' : largestShift.key} changed most in the measured comparison (${largestShift.before} → ${largestShift.after}). Individuals with higher fitness under the recorded environment left more offspring; the environment did not direct the change.`;
  ...
  return (
    <div className="view-stack evolution-view">
      ...
      <div className="comparison-panel"><div className="comparison-heading"><div><p className="eyebrow">BEFORE → AFTER</p><h2>{initial.generation === 0 ? 'Initial population' : `Generation ${initial.generation}`} to generation {current.generation}</h2></div><span className="comparison-arrow" aria-hidden="true">→</span><span className="seed-stamp">SEED {snapshot.seed}</span></div><div className="comparison-table"><div className="comparison-head"><span>Trait</span><span>Initial</span><span>Evolved</span><span>Shift</span></div>{deltas.map((item) => <div className="comparison-row" key={item.key}><span>{item.key === 'coldTolerance' ? 'Cold tolerance' : item.key}</span><strong>{item.before}</strong><strong>{item.after}</strong><b className={item.delta > 0 ? 'delta-up' : item.delta < 0 ? 'delta-down' : ''}>{item.delta > 0 ? '+' : ''}{item.delta}</b></div>)}</div><p className="data-explanation"><span className="explanation-mark">↳</span>{explanation}</p></div>
