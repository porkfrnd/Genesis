...
export default function GenomeRail({ organism, genome, selectedGene, onSelectGene, onChangePair, editable = false, onOpenGenome }) {
  ...
  const currentGenome = genome ?? organism.genome;
  return (
    <section className="genome-rail" aria-labelledby="genome-rail-title">
      <div className="module-heading compact-heading"><div><p className="eyebrow">GENOME / FIVE SIMPLE LOCI</p><h2 id="genome-rail-title">{editable ? 'Choose a gene to change' : 'What genes does it have?'}</h2></div><span className="mono-label">5 LOCI</span></div>
      {!editable && <button className="rail-open-button" type="button" onClick={onOpenGenome}>Open the genome editor <span aria-hidden="true">→</span></button>}
      <div className="locus-list">
        {GENE_DEFINITIONS.map((gene) => {
          const pair = currentGenome[gene.id];
          ...
          return (
            <div className={`locus-row ${selectedGene === gene.id ? 'is-selected' : ''}`} key={gene.id}>
              ...
              {selectedGene === gene.id && <div className="locus-editor"><p className="locus-effect">{gene.alleles[dominant]?.effect}. {gene.tradeoff}</p>{editable ? <div className="allele-controls"><AlleleSelect gene={gene} index={0} value={pair[0]} onChange={(index, allele) => onChangePair(gene.id, index, allele)} /><span aria-hidden="true">/</span><AlleleSelect gene={gene} index={1} value={pair[1]} onChange={(index, allele) => onChangePair(gene.id, index, allele)} /></div> : <span className="dominance-note"><b>Current version:</b> {dominant} · {gene.alleles[dominant]?.label}</span>}{editable && <span className="dominance-note"><b>Expressed:</b> {dominant} · {gene.alleles[dominant]?.label}</span>}</div>}
            </div>
          );
        })}
      </div>
